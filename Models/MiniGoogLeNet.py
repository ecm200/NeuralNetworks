...
from tensorflow.keras.layers import Activation, Dense, Dropout, Flatten, Input, Lambda, concatenate
...
class MiniGoogLeNet:
    ...
    def inception_module(x, numK1x1, numK3x3, chanDim):
        # Define two CONV modules, then concat across the channel dimension
    ...
    def downsample_module(x, K, chanDim):
        # Define the CONV module and POOL, then concatenate across channels dimensions.
    ...
    @staticmethod
    def build(width, height, depth, classes, imgProcFunc=None):
        ...
        ## INPUT BLOCK
        # Define Input and first CONV module
        inputs = Input(shape=inputShape)
        # Pass a user defined image preprocessing function for handling the images 
        # on loading during the batch run.
        if imgProcFunc is not None:
            inputs = Lambda(imgProcFunc)(inputs)
        # CONV => BN => RELU
